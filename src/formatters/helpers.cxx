...
namespace helpers {
	...
	void write_file( string path_filename, string input, bool b_unescape_string, ios_base::openmode mode ) {
	...
	vector<string> comma_seperate( string input, size_t limit_to, bool ignore_in_quotes ) {
	...
	vector<string> equal_seperate( string input, size_t limit_to) {
		vector<string> seperated;
		size_t comma_pos = input.find('=');
		size_t found_counter = 0;
		while(comma_pos != string::npos) {
