...
bool Modem::mmcli_send_sms( string message, string number ) {
	...
	string sms_results = sys_calls::terminal_stdout(this->configs["DIR_SCRIPTS"] + "/modem_information_extraction.sh sms send \"" + helpers::unescape_string(message, '"') + "\" \"" + helpers::remove_char_advanced(number, ' ') + "\" " + this->getIndex());
