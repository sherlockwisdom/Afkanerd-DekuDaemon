...
bool system_check( string path_to_sys_file) {
	...
	string DEKU_MODEM_MONITOR_VALUES = "IMEI VARCHAR(64) NOT NULL PRIMARY KEY,"
		...
		"DATE timestamp DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP",
	       ...
	       DEKU_MODEM_SMS_STATUS_VALUES = "IMEI VARCHAR(64) NOT NULL,"
		       "WORK_LOAD INT(11) NOT NULL,"
		       "DATE DATE,"
		       "MDATE timestamp DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP",

	       DEKU_MODEM_SMS_RECEIVED_VALUES = "IMEI VARCHAR(64) NOT NULL,"
	...
	if( !mysql.has_table( DEKU_TABLE_MODEM_MONITOR ) ) {
		if( mysql.create_table( DEKU_TABLE_MODEM_MONITOR, DEKU_MODEM_MONITOR_VALUES ))
			logger::logger(__FUNCTION__, "MODEM MONITOR TABLE CREATED", "stdout", true);
		else {
